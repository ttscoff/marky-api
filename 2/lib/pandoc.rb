...
class PandocRuby
  ...
  attr_accessor :pandoc_path, :input_files, :input_string
  ...
  attr_writer :binary_output, :options, :option_string, :writer
  ...
  def execute(command)
    # @timeout ||= 31_557_600
    output, exit_status = Open3.capture2(command, stdin_data: input_string)

    #   raise StandardError, "Pandoc failed" unless exit_status && exit_status.success?
