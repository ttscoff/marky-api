...
class HTML2Text(HTMLParser.HTMLParser):
    ...
    def handle_tag(self, tag, attrs, start):
        ...
        if tag == "br" and start: self.o("__BR__\n")
        ...
        if tag == 'dd' and not start: self.p()
        ...
        if tag == 'thead':
            self.table_has_header = 1


        if tag == 'tr':
            if start:
                if self.table == 2 and self.table_has_header == 1:
                    self.o("\n" + ('| ---- ' * self.table_cols_max) + " |\n| ")
                    self.table_has_header = 0
                elif self.table_has_header == 0 and self.table == 0:
                    self.o("\n| ----- |\n| ")
                else:
                    self.o("\n| ")
            else:
...
def normalize_tables(input):
    pattern = r'''(?P<table>(?<=\n)(\|.*?\n)+)'''

    def fix_table(match):
        # return match.group('table')
        # sys.stdout.write(match.group('table'))
        # sys.stdout.write("\n---####____\n")
        return normtable(match.group('table')) + "\n"

    return re.sub(pattern, fix_table, input)

def fixheadlines(input):
    pattern = '(?:\[\n*)?(#+) \[?(.*?)\n+\]([\[\(].*?[\]\)])'
    # $1 [$2]$3

    def fixheadline(match):
        return match.group(1)+' ['+match.group(2)+']'+match.group(3)

    # single replacement
    text = re.sub(pattern, fixheadline, input, re.S)
    return text

def fixbrackets(input):
    pattern = '(\s*)(`|[\*_]+)\[([^\]]+\\1\])'
    # $1 [$2]$3

    def fixbracket(match):
        return match.group(1)+'['+match.group(2)+match.group(3)

    # single replacement
    text = re.sub(pattern, fixbracket, input, re.S)
    return text
...
def main():
    ...
    # wrapwrite(fixheadlines(normalize_tables(h.handle(data.replace(u"\u201c", "\"").replace(u"\u201d", "\"").replace(u"\u2018","'").replace(u"\u2019","'")))))
    wrapwrite(fixheadlines(fixbrackets(h.handle(data.replace(u"\u201c", "\"").replace(u"\u201d", "\"").replace(u"\u2018","'").replace(u"\u2019","'")))))
